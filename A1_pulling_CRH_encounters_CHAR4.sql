...
Dependencies:  [PACT_CC].[CRH].[CRH_sites_FY20_working];  [PACT_CC].[CRH].[CRH_sites_FY21_working];  [PACT_CC].[CRH].[CRH_sites_FY22_partial]
...
WHERE A.NationalCHAR4 IN ('DMDC', 'DMEC', 'DMFC', 'DMGC', 'DMJC', 'DMKC', 'DMLC', 'DMQC', 'DMSC', 'DMRC','DMAC',--original set
'CDBC', 'CDGC', 'CDFC', 'CDEC', 'CDHC', 'CDSC', 'CDJC', 'CGEC', 'CGFC', 'CGNC', 'CGKC', 'CGPC', 'CGQC', 'CGWC', --the next lines are from the Approved CHAR4 Codes .xlsx
'CGUC', 'CNSP', 'DEBC', 'DEDC', 'DEEC', 'DEFC', 'DELC', 'DEMC', 'DENC', 'DEPS', 'DEQC', 'DERC', 'DETC', 'DEVC', 
'DEWC', 'DMAC', 'DMBC', 'DESC', 'DMEC', 'DMFC', 'DMGC', 'DMJC', 'DMKC', 'DMLC', 'DMQC', 'DMRC', 'DMSC', 'DMVC', 
'DMWC', 'HTBC', 'NASP', 'SCHC', 'SCVT', 'SNVC', 'WCRC')
--38,601
...
--FY24
drop table if exists #fy24_deduplicated;
WITH cte_flag as (
SELECT
Hub_Region,
Hub_VISN,
...
select min(visitdate) from #fy20_utilization
--576,585
...
drop table if exists [PACT_CC].[CRH].A1_crh_char4_utilization_new;
--
SELECT *
INTO [PACT_CC].[CRH].A1_crh_char4_utilization_new
